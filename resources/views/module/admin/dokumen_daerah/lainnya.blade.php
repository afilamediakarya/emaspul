...
@section('button')
    @if($role == 1)
    <button class="btn btn_general btn-sm " data-kt-drawer-show="true" data-kt-drawer-target="#side_form" id="button-side-form"><i class="fa fa-plus-circle" style="color:#ffffff" aria-hidden="true"></i> Tambah Dokumen</button>
    @endif
@endsection
@section('content')
...
<script>
    ...
    $(document).on('click','.button-show', function (e) {
        e.preventDefault();
        window.open('/storage/files/dokumen_daerah/daerah/'+$(this).attr('data-label'), '_blank');
    })
    ...
    $(function () {
        ...
        control.initDatatable('/general/datatable-list?jenis=10&type=type_a&type_query=query_2',columns,columnDefs);
        control.form_upload();
